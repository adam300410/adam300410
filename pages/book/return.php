...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        // 更新 book_info2 表格
        $update_sql = "UPDATE book_info2 SET available = 1 WHERE id = ?";
        $update_stmt = $pdo->prepare($update_sql);
        $update_stmt->execute([$book_id]);

        // 記錄還書時間
        $return_date_time = date('Y-m-d H:i:s');
        $update_borrow_sql = "UPDATE book_info SET return_date_time = ? WHERE id = ?";
        $update_borrow_stmt = $pdo->prepare($update_borrow_sql);
        $update_borrow_stmt->execute([$return_date_time, $borrow_info['id']]);

        echo "<script>alert('還書成功！'); window.location.href = 'search.php';</script>";
        exit;
    }
